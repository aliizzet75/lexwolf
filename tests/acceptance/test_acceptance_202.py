...
def test_xunit_test_fuer_alias_kette_vorhanden():
    src = _sources()
    assert "RepeatedAnonymizationTests" in src, "Regressionstest fuer Task #202 fehlt"
    assert "[Fact]" in src and "AnonymizeText" in src
    for marker in ("Watson", "Moriarty", "STEUER-", "ORT-"):
        assert marker in src, f"Test muss Fall '{marker}' abdecken"


def test_dotnet_test_alias_kette_ist_gruen():
    result = subprocess.run(
        ["dotnet", "test", _TEST_PROJ, "--filter", "FullyQualifiedName~RepeatedAnonymizationTests"],
        cwd=_ANON_DIR, capture_output=True, text=True, timeout=300,
    )
    assert result.returncode == 0, (
        f"dotnet test schlägt fehl (erwartet, solange Task #202 nicht implementiert ist):\n"
